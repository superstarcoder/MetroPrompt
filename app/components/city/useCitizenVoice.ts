...
//
// Deepgram owns the whole speech loop — STT (Flux), LLM (Claude Haiku 4.5 with
// the citizen persona), and TTS (Aura-2) — including turn detection and
// barge-in. We only move audio in and out, and mirror the state it reports.
//
// Audio contract, fixed by the Settings message below:
//   mic  → linear16, 16 kHz, mono   (we downsample via the AudioContext)
...
export type CitizenVoice = {
  ...
  /** Mic gated off: the citizen cannot hear you, and cannot be barged in on. */
  muted: boolean;
  /**
   * What the citizen is saying right now, revealed in step with the audio.
...
export function useCitizenVoice(citizen: Person | null): CitizenVoice {
  ...
  const playChunk = useCallback((pcm: ArrayBuffer) => {
    ...
    // First chunk of a new utterance: this is the instant the citizen's voice
    // actually begins, so it's the zero point the transcript reveals from.
  ...
  const start = useCallback(async () => {
    if (!citizen || runningRef.current) return;
    ...
    try {
      ...
      const cfg: {
        token: string; prompt: string; voice: string; keyterms: string[]; greeting: string;
      } = await res.json();
      ...
      // attached later would miss the citizen's first words.
      socket.addEventListener('message', (ev: MessageEvent) => {
        ...
        switch (m.type) {
          case 'UserStartedSpeaking':
            ...
            // already on screen is exactly what the citizen got to say before
          ...
          agent: {
            listen: {
              provider: {
                ...
                // Citizen name, employer, and the places they've actually been.
  ...
  // The work belongs in the cleanup rather than the effect body: it runs for
  // the OUTGOING citizen, which is exactly whose socket and mic we want closed.
  useEffect(() => {
    return () => {
      teardown();
      setStatus('idle');
