...
// "Hooli (Office)" / "Sakura Ramen (Restaurant)" for named businesses;
// otherwise the plain type label.
export function formatPropertyLabel(p: Property | null | undefined): string {
  if (!p) return '—';
  const label = PROPERTY_LABELS[p.name] ?? p.name;
  if (p.company_name) return `${p.company_name} (${label})`;
  return label;
}
...
export function formatTripDestination(t: Pick<Trip, 'destination_name' | 'destination_company'>): string {
  ...
  if (t.destination_company) return `${t.destination_company} (${label})`;
  return label;
}
